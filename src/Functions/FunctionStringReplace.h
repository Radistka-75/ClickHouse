...
public:
    ...
    const bool enable_vectorscan_in_replace_regexp;
    static FunctionPtr create(ContextPtr context)
    {
        const auto & settings = context->getSettingsRef();
        return std::make_shared<FunctionStringReplace>(settings.enable_vectorscan_in_replace_regexp);
    }

    explicit FunctionStringReplace(bool enable_vectorscan_in_replace_regexp_)
        : enable_vectorscan_in_replace_regexp(enable_vectorscan_in_replace_regexp_)
    {}
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override
    {
        ...
        if (const ColumnString * col = checkAndGetColumn<ColumnString>(column_src.get()))
        {
            ...
            Impl::vector(col->getChars(), col->getOffsets(), needle, replacement, col_res->getChars(), col_res->getOffsets(), enable_vectorscan_in_replace_regexp);
            return col_res;
        }
        else if (const ColumnFixedString * col_fixed = checkAndGetColumn<ColumnFixedString>(column_src.get()))
        {
            auto col_res = ColumnString::create();
            Impl::vectorFixed(col_fixed->getChars(), col_fixed->getN(), needle, replacement, col_res->getChars(), col_res->getOffsets(), enable_vectorscan_in_replace_regexp);
            return col_res;
        }
        else
            throw Exception(
