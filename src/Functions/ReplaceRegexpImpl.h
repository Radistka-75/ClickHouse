...
struct ReplaceRegexpImpl
{
        ...
        const Instructions & instructions)
    {
                           ...
                           void * context_) -> int
        {
            ...
            if constexpr (replace == ReplaceRegexpTraits::Replace::First)
                return 1; /// Cease matching (HS_SCAN_TERMINATED)
            else
                return 0;
        };
    ...
    static void vector(
        ...
        bool enable_vectorscan_in_replace_regexp)
    {
        ...
        if (num_captures_in_needle == -1)
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS,
                "The pattern argument is not a valid re2 pattern.");

        // TODO factorize for reuse in vectorFixed()
        ...
        ///
        /// Also, evaluation with flag HS_FLAG_SOM_LEFTMOST may reject some complex but valid patterns
        /// according to vectorscan documentation. Therefore, allow to turn vectorscan on/off via cfg.
        bool use_vectorscan = (enable_vectorscan_in_replace_regexp && num_captures_in_needle == 0);
        if (use_vectorscan)
        {
            /// Can't use the internal regexp cache, i.e. construct database + scratch area directly, because we rely on the
            /// HS_FLAG_SOM_LEFTMOST flag here and the other usages of vectorscan in ClickHouse don't.

            hs_database_t * db = nullptr;
            hs_compile_error_t * compile_error = nullptr;

            // TODO need to set the horizon flag??
            ...
            Instructions instructions = createInstructions(replacement, num_captures_in_needle /* allow capture "\0" */ + 1);
...
#endif
        num_captures_in_needle = std::min(num_captures_in_needle + 1, max_captures_re2);
    ...
    static void vectorFixed(
        ...
        ColumnString::Chars & res_data,
        ColumnString::Offsets & res_offsets,
        [[maybe_unused]] bool enable_vectorscan_in_replace_regexp)
    {
        ColumnString::Offset res_offset = 0;
